...
from taiwan_einvoice.models import (
    ...
    Seller,
    TurnkeyService,
    SellerInvoiceTrackNo, NotEnoughNumberError, UsedSellerInvoiceTrackNoError, E0402HasZeroBlankNoError, ExcutedE0402UploadBatchError, ExistedE0402UploadBatchError,
...
PRINT_EI_BOTH_MOBILE_BARCODE_AND_BUYER_IDENTIFIER_HOURS = 36
...
class ESCPOSWebModelViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if res:
            ...
            for _p in CanEditESCPOSWebOperator.ACTION_PERMISSION_MAPPING.get(self.action, []):
            ...
            if res:
                ...
                objs = get_objects_for_user(request.user,
                                            CanOperateESCPOSWebOperator.ACTION_PERMISSION_MAPPING.get(self.action, []),
                                            any_perm=True)
                return objs
...
class ESCPOSWebOperatorModelViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if res:
            ...
            objs = get_objects_for_user(request.user,
                                        CanEditESCPOSWebOperator.ACTION_PERMISSION_MAPPING.get(self.action, []),
                                        any_perm=True)
            return objs
...
class SellerInvoiceTrackNoModelViewSet(ModelViewSet):
    ...
    def create_and_upload_blank_numbers(self, request, pk=None):
        ...
        if sitn:
            ...
            except E0402HasZeroBlankNoError as e:
...
class EInvoiceModelViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not request.user.teastaffprofile or not request.user.teastaffprofile.is_active:
            ...
            permissions = CanEntryEInvoice.ACTION_PERMISSION_MAPPING.get(self.action, [])
            turnkey_services = get_objects_for_user(request.user, permissions, any_perm=True)
            ...
            if request.GET.get('re_print_original_copy', False) in ['true', '1']:
                ...
                if '3J0002' == ei.carrier_type and LegalEntity.GENERAL_CONSUMER_IDENTIFIER != ei.buyer_identifier:
                    ...
                    if not ei.print_mark:
                        if NOW - ei.generate_time < datetime.timedelta(hours=PRINT_EI_BOTH_MOBILE_BARCODE_AND_BUYER_IDENTIFIER_HOURS):
                            pass #TODO: TRADE#377-1
                        else:
                            return Response({"error_title": _("Print E-Invoice Error"),
                                            "error_message": _("The E-Invoice that both set mobile-barcode and buyer identifier can not print out if they are not printed within {} hours of generation.").format(PRINT_EI_BOTH_MOBILE_BARCODE_AND_BUYER_IDENTIFIER_HOURS),
                                            }, status=status.HTTP_403_FORBIDDEN)
...
class EInvoicePrintLogModelViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not request.user.teastaffprofile or not request.user.teastaffprofile.is_active:
            ...
            permissions = CanEntryEInvoicePrintLog.ACTION_PERMISSION_MAPPING.get(self.action, [])
            turnkey_services = get_objects_for_user(request.user, permissions, any_perm=True)
...
class CancelEInvoiceModelViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not request.user.teastaffprofile or not request.user.teastaffprofile.is_active:
            ...
            permissions = CanEntryCancelEInvoice.ACTION_PERMISSION_MAPPING.get(self.action, [])
            turnkey_services = get_objects_for_user(request.user, permissions, any_perm=True)
            return queryset.filter(einvoice__seller_invoice_track_no__turnkey_service__in=turnkey_services)
...
class VoidEInvoiceModelViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not request.user.teastaffprofile or not request.user.teastaffprofile.is_active:
            ...
            permissions = CanEntryVoidEInvoice.ACTION_PERMISSION_MAPPING.get(self.action, [])
            turnkey_services = get_objects_for_user(request.user, permissions, any_perm=True)
            return queryset.filter(einvoice__seller_invoice_track_no__turnkey_service__in=turnkey_services)
...
class UploadBatchModelViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not request.user.teastaffprofile or not request.user.teastaffprofile.is_active:
            ...
            permissions = CanViewTEAlarmForProgrammer.ACTION_PERMISSION_MAPPING.get(self.action, [])
            turnkey_services = get_objects_for_user(request.user, permissions, any_perm=True)
            return queryset.filter(turnkey_service__in=turnkey_services)
...
class SummaryReportModelViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not request.user.teastaffprofile or not request.user.teastaffprofile.is_active:
            ...
            permissions = CanViewSummaryReport.ACTION_PERMISSION_MAPPING.get(self.action, [])
            turnkey_services = get_objects_for_user(request.user, permissions, any_perm=True)
            return queryset.filter(turnkey_service__in=turnkey_services)
...
class TEAlarmModelViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not request.user.teastaffprofile or not request.user.teastaffprofile.is_active:
            ...
            programmer_permissions = CanViewTEAlarmForGeneralUser.ACTION_PERMISSION_MAPPING.get(self.action, [])
            general_user_permissions = CanViewTEAlarmForProgrammer.ACTION_PERMISSION_MAPPING.get(self.action, [])
